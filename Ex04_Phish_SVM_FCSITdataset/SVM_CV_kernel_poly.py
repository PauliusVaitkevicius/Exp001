...
model_label = "SVM (Polynomial kernel with d=" + str(degree) + ")"
print("Training the " + model_label)

svm = SVC(kernel='poly', degree=degree, gamma='auto')

scores = cross_val_score(svm, X, np.ravel(y, order='C'), cv=cv, scoring='accuracy', n_jobs=-1)
# saving results to files
np.savetxt('../results/SVM_Poly_D2_CV30_normal.txt', scores)
scores.tofile('../results/SVM_Poly_D2_CV30_normal.dat')
