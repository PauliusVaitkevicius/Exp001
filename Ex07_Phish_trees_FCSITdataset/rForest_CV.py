...
rf = RandomForestClassifier(criterion='entropy', n_jobs=-1, n_estimators=n_estimators, max_depth=max_depth)

scores = cross_val_score(rf, X, np.ravel(y, order='C'), cv=cv, scoring='accuracy')

# saving results to files
np.savetxt('../results/Random_Forest_CV30_normal.txt', scores)
scores.tofile('../results/Random_Forest_CV30_normal.dat')
